<?php
/**
 * site1 — shared helpers: PDO connection, schema bootstrap, localhost guard,
 * and the educational disclaimer banner. Part of the CyberForge thesis test lab.
 */
...
/** Connect to MySQL (PDO). Supports a DATABASE_URL or discrete DB_* settings. */
function db(): PDO {
    static $pdo = null;
    if ($pdo) { return $pdo; }
    $c = cfg();
...
/** Create the capture table on first run so no manual import is needed. */
...
/** Refuse to serve off-loopback unless explicitly allowed — keeps this a lab. */
function require_local(): void {
    ...
    if (!$local) {
        ...
        echo '<!doctype html><meta charset="utf-8"><body style="font-family:system-ui;background:#0b0f17;color:#e7edf5;padding:48px">'
           . '<h2>403 — Localhost only</h2><p>This is an educational phishing <em>simulation</em> for the CyberForge thesis and only runs on localhost. '
           . 'Set <code>ALLOW_REMOTE=1</code> if you intentionally need otherwise.</p></body>';
        exit;
    }
}
...
/** The honest footer shown on every page. */
function sim_banner(): string {
    return '<div style="position:fixed;left:0;right:0;bottom:0;z-index:9999;background:#7a1020;color:#ffd9df;'
        ...
        . '⚠ Educational phishing <u>simulation</u> — CyberForge thesis lab. Localhost only. Enter fake data only.'
        . '</div>';
}
