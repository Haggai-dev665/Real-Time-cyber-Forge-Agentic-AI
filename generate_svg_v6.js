...
function rect(cx, cy, w, h, textLines, bgColor, strokeColor, sw=2, rx=8) {
    if (!bgColor) bgColor = "#e3f2fd";
    if (!strokeColor) strokeColor = "#1e88e5";
    
    let texts = textLines.map(function(t, i) {
...
function line(pathStr, label="", lX=0, lY=0, arrow=true) {
    ...
    let route = '<path d="' + pathStr + '" fill="none" stroke="#424242" stroke-width="2.5"' + marker + ' />';
    if (label) {
...
out += rect(600, 60, 220, 50, ["User Browses Web"], USER_DEF[0], USER_DEF[1], 2, 25) + "\\n";
out += rect(600, 140, 260, 60, ["Tauri Desktop App", "Intercepts URL Activity"], BOX_DEF[0], BOX_DEF[1]) + "\\n";
out += line("M 600,85 L 600,110") + "\\n";

// Level 2: Backend Orchestrator & Fast Checks
out += rect(600, 230, 280, 60, ["Node.js Orchestrator API", "Receives Telemetry"], BOX_DEF[0], BOX_DEF[1]) + "\\n";
out += line("M 600,170 L 600,200") + "\\n";

out += diamond(600, 330, 180, 80, ["URL in Cache?"]) + "\\n";
out += line("M 600,260 L 600,290") + "\\n";

out += rect(250, 330, 180, 60, ["Return Cached", "Verdict immediately"], DB_DEF[0], DB_DEF[1]) + "\\n";
...
out += rect(250, 450, 220, 60, ["Mark URL Safe", "Avoids False Positive Noise"], BOX_DEF[0], BOX_DEF[1]) + "\\n";
out += line("M 480,450 L 360,450", "Yes", 420, 440) + "\\n";

// Level 3: Stage 1 Agents (Parallel)
out += rect(600, 560, 300, 50, ["DAG Pipeline Execution Triggered"], BOX_DEF[0], BOX_DEF[1]) + "\\n";
out += line("M 600,490 L 600,535", "No", 600, 515) + "\\n";

out += rect(300, 650, 200, 60, ["URL Phishing BERT", "FastAPI Hugging Face Space"], AI_DEF[0], AI_DEF[1]) + "\\n";
out += rect(600, 650, 200, 60, ["DGA Detector", "Entropy Heuristics"], BOX_DEF[0], BOX_DEF[1]) + "\\n";
out += rect(900, 650, 200, 60, ["Web Scraper", "Headless Scrapper.live"], DB_DEF[0], DB_DEF[1]) + "\\n";
...
out += rect(600, 780, 320, 60, ["Wait for DOM Scrape Result", "Send to Deep Content Analyzers"], BOX_DEF[0], BOX_DEF[1]) + "\\n";
out += line("M 300,680 L 550,750") + "\\n";
out += line("M 600,680 L 600,750") + "\\n";
out += line("M 900,680 L 650,750") + "\\n";

out += rect(300, 880, 200, 60, ["IOC Extractor", "(IPs, Hashes, Links)"], BOX_DEF[0], BOX_DEF[1]) + "\\n";
out += rect(600, 880, 200, 60, ["Behavioral Analyzer", "Examine JS / Network"], BOX_DEF[0], BOX_DEF[1]) + "\\n";
out += rect(900, 880, 200, 60, ["MITRE TTP Mapper", "Align to Cyber Kill Chain"], BOX_DEF[0], BOX_DEF[1]) + "\\n";
...
out += rect(600, 1000, 280, 60, ["AlienVault OTX Enrichment", "Correlate Extracted IOCs"], DB_DEF[0], DB_DEF[1]) + "\\n";
out += line("M 300,910 L 550,970") + "\\n";
out += line("M 600,910 L 600,970") + "\\n";
out += line("M 900,910 L 650,970") + "\\n";

// Final Aggregation
out += rect(600, 1120, 280, 60, ["Aggregate Scores & Findings", "(Weighting Algorithm)"], BOX_DEF[0], BOX_DEF[1]) + "\\n";
...
out += rect(300, 1360, 240, 70, ["Trigger UI Alert!", "Notify user instantly/protects"], ALERT_DEF[0], ALERT_DEF[1], ALERT_DEF[2]) + "\\n";
out += line("M 490,1240 L 300,1240 L 300,1325", "Yes (Threat Found)", 395, 1230) + "\\n";

out += rect(300, 1480, 280, 70, ["LLM Chain", "Generate Explainable Narrative"], AI_DEF[0], AI_DEF[1]) + "\\n";
out += line("M 300,1395 L 300,1445") + "\\n";

// Clean Flow
out += rect(900, 1360, 240, 60, ["Mark URL Safe", "Seamless browsing experience"], DB_DEF[0], DB_DEF[1]) + "\\n";
out += line("M 710,1240 L 900,1240 L 900,1330", "No (Clean / Safe)", 805, 1230) + "\\n";

// Connect to Persist
out += rect(600, 1620, 280, 60, ["Save Alert Data to Appwrite", "Update Redis Cache"], DB_DEF[0], DB_DEF[1]) + "\\n";
